...
## Indentation
  ...
  - Always use 2 spaces in your indentation. In some scripts, like sass, it will show errors if we use both tabs and spaces
  - Every statement should begin aligned with the current indentation.
  - The outermost level is at the left margin. The indentation increases by 4 spaces when the last token on the previous line is { left brace, [ left bracket, ( left paren. The matching closing token will be the first token on a line, restoring the previous indentation.
  
  - There should be space after if or while.
    ...
    while (true) {
    ```
  - ? question mark always begins a line and increase the indentation by 4 spaces, 
  - : colon always begins a line, aligned with the ? question mark. 
  - The condition should be wrapped in parens.
